...
get_outlier_scores = function(network, k, cluster = FALSE, cluster_n = 5) {
  ...
  # for each day, get number of times it was tagged as an outlier (to use for
  # voting)
  ...
  # resid = do.call(cbind, lapply(out_list, getElement, name = 'resid'))
  # colnames(resid) = neighbors
  # list(high = high_scores, low = low_scores, resid = resid)
  out
}
...
# we're raising `k` because the residuals are smaller, now that we're predicting
# with neighboring loads instead of TV
...
# ok, can we use mvoutliers
# library(mvoutlier)
# has_nas = sapply(reg_data[, -1], function(x) any(is.na(x)))
# x = pcout(reg_data[, -1][, !has_nas], makeplot = TRUE)
# apply(reg_data[, -1], 2, mad)
# sapply(reg_data[, -1], function(x) sum(is.na(x))) %>%
#   sort(decreasing = T)

with(reg_data, plot(reading.10B, reading.32M, type = 'b'))

library(rrcovNA)
library(ggplot2)

x2 = PcaNA(reg_data[, -1], k = 2)

plot(x2)

x_ = x2$scores %*% t(x2$loadings)
for (i in seq_len(ncol(x_))) x_[, i] = x_[, i] + x2$center[i]
diffs = as.matrix(reg_data[, -1]) - x_

hist(as.matrix(reg_data[, -1]) - x_)

plot(reg_data$day, diffs[, 'reading.32M'])

# get a robust PCA fit for each borough
pca_fits = reg_data %>%
  subset(select = -c(day, year)) %>%
  names %>%
  sub('reading\\.[0-9]*', '', .) %>%
  unique %>%
  lapply(function(x) {
    borough_cols = endsWith(names(reg_data), x)
    PcaNA(reg_data[, borough_cols], k = 1, method = 'hubert')
  })
names(pca_fits) = reg_data %>%
  subset(select = -c(day, year)) %>%
  names %>%
  sub('reading\\.[0-9]*', '', .) %>%
  unique
...
# get the scaled residual from the PCA-reconstructed readings
get_scaled_resid = function(comps, network) {
  network_col = paste0('reading.', network)
  readings_pca = (comps$scores %*% t(comps$loadings))[, network_col]
  readings_pca = readings_pca + x2$center[network_col]
  diffs = reg_data[, network_col] - readings_pca
  diffs_y = diffs - ave(diffs, reg_data$year, FUN = median)
  diffs_y / mad(diffs_y, na.rm = TRUE)
}
...
reg_data %>%
  subset(select = -c(day, year)) %>%
  names %>%
  sub('reading.', '', ., fixed = TRUE) %>%
  lapply(function(x) {
    borough = sub('[0-9]*', '', x)
    borough_pca = pca_fits[[borough]]
    network_col = paste0('reading.', x)
    data.frame(day = reg_data$day,
               network = x,
               resid = get_scaled_resid(borough_pca, x),
               flagged = is.na(old_reg_data[match(reg_data$day, old_reg_data$day), network_col]))
  }) %>%
  do.call(rbind, .) %>%
  subset(endsWith(network, 'R')) %>%
  ggplot(aes(day, resid, color = flagged, group = 1)) +
  geom_point() +
  geom_line() +
  facet_wrap(~ network, scales = 'free_y')

# let's organize this by year
reg_data %>%
  transform(year = trunc(day, 'year')) %>%
  transform(diffs_32M = diffs[, 'reading.32M']) %>%
  transform(diffs_32M_y = diffs_32M - ave(diffs_32M, year, FUN = median)) %>%
  merge(old_reg_data[, c('day', 'reading.32M')], by = 'day', all.x = TRUE) %>%
  # names
  # subset(year == '2024-01-01') %>%
  # with(plot(day, diffs_32M_y))
  ggplot(aes(day, diffs_32M_y, color = is.na(reading.32M.y), group = 1)) +
  geom_point() +
  geom_line()
...
# for comparison
o4 = readRDS('results/load_vs_weather/load_outliers.rds')
old_reg_data = readRDS('results/load_vs_weather/tv_and_load.rds')
...
# this is what I used in the load_vs_weather script
o4 = get_network_outliers(k = 2.5, cluster_k = 1, cluster_n = 5)

# manually remove some more outliers, update `o4` with these new periods removed

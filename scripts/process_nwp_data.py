...
nwp_dir = 'scripts/data/nwpdownload'
...
def get_all_0p5(ds_string):
    ...
    '''
    nc_0p5b = nwp_dir + f'/gefs_atmos0p5b_{ds_string}_members.nc'
    nc_0p5 = nwp_dir + f'/gefs_atmos0p5_{ds_string}.nc'
...
def backfill_fct(fct, anl):
    ...
    '''
    pre_weathers = [ get_pre_weather(anl, t) for t in list(fct['time'].values) ]
...
gefs_0p25_fct = xr.open_dataset(nwp_dir + '/gefs_atmos0p25_fct.nc').\
    squeeze('number').\
    drop_vars('number')

# organize the analysis
gefs_0p25_anl = xr.open_dataset(nwp_dir + '/gefs_atmos0p25_anl.nc').\
    squeeze().\
    drop_vars('step')
# add the variables not in the analysis files
gefs_0p25_fill6 = xr.open_dataset(nwp_dir + '/gefs_atmos0p25_fill6.nc').\
    squeeze().\
    drop_vars('step')
...
gefs_0p25_fill3 = xr.open_dataset(nwp_dir + '/gefs_atmos0p25_fill3.nc').\
    squeeze().\
    drop_vars('step')
...
gefs_atmos0p25_fct_members = xr.open_dataset(nwp_dir + '/gefs_atmos0p25_fct_members.nc')
...
gefs_tv_members.to_netcdf(out_dir + '/gefs_tv_members.nc')
